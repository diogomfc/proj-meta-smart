import React, { ReactNode, useState, FormEvent, useEffect } from "react";
...
type IDbLinks = {
  ...
  >;
  dbLinksCommitsLikes?: Record<
    string,
    {
      likesAuthorId?: string;
    }
  >;
};
...
function BasedeConhecimento() {
  ...
  const [dbGeral, setdbGeral] = useState<IDbLinks[]>([]);

  //? TESTES TESTES FIRESTORE */
  ///////// teste fireSotre //////////////////////////////////
  type CadastroUsers = {
    id: string;
    name: string;
  ...
  const [buscaFirestore, setBuscaFirestore] = useState<CadastroUsers[]>([]);

  function cadastroFirestore() {
    console.log("Olá");
  ...
  const buscaCadastroFirestore = useEffect(() => {
    dbFirestore.collection("users").onSnapshot((snapshot) => {
      const dados: CadastroUsers[] = [];

      const lista = snapshot.forEach((doc) => {
        const data = doc.data();
        ...
        setBuscaFirestore(dados);
      });
    });
  }, []);

  const excluirCadastroFirestore = ({ id, name }: CadastroUsers) => {
    if (confirm(`deseja realmente excluir o cadastro ${name}`)) {
  ...
  const editarCadastroFirestore = (banana: CadastroUsers) => {
    banana.id;
  };
  //? TESTES TESTES FIRESTORE */
  ...
  useEffect(() => {
    commitsRef.on("value", (commits) => {
      ...
      console.log(`Log Cadastro useEffect Commits - ${dbCadastroCommits}`);
    });
    return () => {
  ...
  useEffect(() => {
    linkRef.on("value", (result) => {
      ...
      console.log(`Log Cadastro useEffect GERAL - ${resultCadastroLinks}`);
    });
    return () => {
      linkRef.off("value");
    };
